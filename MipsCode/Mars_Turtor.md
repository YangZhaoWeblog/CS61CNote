...
1. 高级语言中数组的工作原理, 基本是采用寄存器相对寻址.  即：  把数组的首地址放到寄存器中, 数组下标值作为相对偏移.
2. 将数组元素按照访问顺序放在存储容器中，能得到更好的性能.

3. 分支预测
> 在18世纪，火车在岔路口的时候不会停下来选择正确方向后再前进，而会先选择认为对的方向前进，如果错了再倒回去选择真正正确的方向。这样做的好处是: 如果你一直猜对了，你将一直前行!! 

遍历并访问未排序的数组中的元素时的时间 >> 要远远大于 遍历并访问已排序的数组中的元素时的时间
这 分支的 (branch predict) 的较坏情况

# c 与 mips
```bash
mips-linux-gnu-gcc -S -O2 -fno-delayed-branch -I/usr/include InsertSort.c -o InsertSort.s  
```
